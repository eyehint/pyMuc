        ...
        for target in self.target:
            ...
            if c == 1 and target.checkConfig('자동습득') == True:
                ...
                if self['레벨'] >= 2000 and chance < 1:
                    ...
                    if item != None:
                        if target.getItemCount() <= getInt(MAIN_CONFIG['사용자아이템갯수']) and target.getItemWeight() + item[
                            '무게'] < target.getStr() * 10:
                            obj = item.deepclone()
                            if randint(0, 99) < 30:
                                obj.applyMagic(self['레벨'], 0)
                ...
                for item in objs:
                    if target.getItemCount() > getInt(MAIN_CONFIG['사용자아이템갯수']) or target.getItemWeight() + item[
                        '무게'] > target.getStr() * 10:
                        break
                    self.remove(item)
                    target.insert(item)
